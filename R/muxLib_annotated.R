...
GetLargestEigenv <- function(Matrix) {
  ...
  tmp$vectors[Matrix::which(tmp$vectors > -1e-12 & tmp$vectors < 1e-12)] <-
    0
...
#'    on inter-layer coupilng \eqn{D^{\alpha \beta}_{i}\}_i}.
...
BuildSupraTransitionMatrixFromSupraAdjacencyMatrix <-
  function(SupraAdjacencyMatrix,
    ...
    DisconnectedNodes <-
      length(SupraStrengthMatrix[SupraStrengthMatrix == 0])
        ...
        if (in_layer_disconnected_count > 0) {
          ...
          for (beta in 1:Layers) {
            ...
            if (length(is) == 1) {
              ...
              diag(SupraTransitionMatrix[is, in_layer_disconnected_ids]) <- 1 / CouplingStrengthVector[is]
            }
          }
        }
      }
      if (Type == "relaxed-physical") {
        ...
        if (in_layer_disconnected_count > 0) {
          ...
          if (length(in_layer_disconnected_ids) > 1) {
            diag(SupraTransitionMatrix[in_layer_disconnected_ids, in_layer_disconnected_ids]) <- (1 - r) * 1
          } else {
...
#' @param Approximate logical, use approximate method
#' @param Approximate.disconnected Numeric for approximate method
#' @return
...
GetCoverageEvolutionMultilayer <- function(SupraTransitionMatrix, 
  ...
  # I don't use the SolveEigenvalueProblem function because I expect complex numbers in the process
  
  if (Approximate.disconnected == 0)
    Approximate.disconnected <- Order
  
  # this can not be done, because we need the invQ later,
  # and invQ = Matrix::t(Q) only if the laaplacian is symmetric..
  ...
  cat(paste(
    "  :: 90% difference expected for tau >",
    -log(1 - 0.9) / (lambdas.sort[2] - lambdas.sort[1]),
    "\n"
  ))
